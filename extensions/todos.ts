...
const TodoParams = Type.Object({
	...
	parent_id: Type.Optional(
		Type.String({ description: "Parent todo id. Set on create/update to make this todo a subtask." }),
	),
...
function buildTodoHierarchy(
	todos: TodoFrontMatter[],
): Map<string, TodoFrontMatter[]> {
	const childrenByParent = new Map<string, TodoFrontMatter[]>();
	for (const todo of todos) {
		const parentId = todo.parent_id || "__root__";
...
function getSubtaskStats(
	todoId: string,
	childrenByParent: Map<string, TodoFrontMatter[]>,
): { total: number; closed: number } {
	const children = childrenByParent.get(todoId) || [];
	const closed = children.filter((child) => isTodoClosed(child.status)).length;
	return { total: children.length, closed };
}

function flattenTodoHierarchy(
	todos: TodoFrontMatter[],
	childrenByParent: Map<string, TodoFrontMatter[]>,
): Array<{ todo: TodoFrontMatter; depth: number; subtaskStats?: { total: number; closed: number } }> {
	const todoById = new Map(todos.map((todo) => [todo.id, todo]));
	const roots = todos.filter((todo) => !todo.parent_id || !todoById.has(todo.parent_id));
	const result: Array<{ todo: TodoFrontMatter; depth: number; subtaskStats?: { total: number; closed: number } }> = [];
	const visited = new Set<string>();

	const traverse = (todo: TodoFrontMatter, depth: number) => {
		if (visited.has(todo.id)) return;
		visited.add(todo.id);
		const stats = getSubtaskStats(todo.id, childrenByParent);
		result.push({
			todo,
			depth,
			subtaskStats: stats.total > 0 ? stats : undefined,
		});
...
class TodoSelectorComponent extends Container implements Focusable {
	...
	private flattenedFilteredTodos: Array<{
		todo: TodoFrontMatter;
		depth: number;
		subtaskStats?: { total: number; closed: number };
	}>;
	...
	constructor(
		...
		this.searchInput.onSubmit = () => {
			const selected = this.flattenedFilteredTodos[this.selectedIndex]?.todo;
	...
	private applyFilter(query: string): void {
		...
		const childrenByParent = buildTodoHierarchy(this.filteredTodos);
		this.flattenedFilteredTodos = flattenTodoHierarchy(this.filteredTodos, childrenByParent);
		this.selectedIndex = Math.min(this.selectedIndex, Math.max(0, this.flattenedFilteredTodos.length - 1));
		this.updateList();
	}
	...
	private updateList(): void {
		...
		if (this.flattenedFilteredTodos.length === 0) {
		...
		const startIndex = Math.max(
			0,
			Math.min(this.selectedIndex - Math.floor(maxVisible / 2), this.flattenedFilteredTodos.length - maxVisible),
		);
		const endIndex = Math.min(startIndex + maxVisible, this.flattenedFilteredTodos.length);

		for (let i = startIndex; i < endIndex; i += 1) {
			const row = this.flattenedFilteredTodos[i];
			if (!row) continue;
			const { todo, depth, subtaskStats } = row;
			...
			const hierarchyPrefix = depth > 0 ? `${"  ".repeat(depth - 1)}↳ ` : "";
			const subtaskText = subtaskStats
				? this.theme.fg("muted", ` ${subtaskStats.closed}/${subtaskStats.total} subtasks`)
				: "";
			const assignmentText = renderAssignmentSuffix(this.theme, todo, this.currentSessionId);
			const line =
				prefix +
				this.theme.fg("dim", hierarchyPrefix) +
		...
		if (startIndex > 0 || endIndex < this.flattenedFilteredTodos.length) {
			const scrollInfo = this.theme.fg(
				"dim",
				`  (${this.selectedIndex + 1}/${this.flattenedFilteredTodos.length})`,
			);
	...
	handleInput(keyData: string): void {
		...
		if (kb.matches(keyData, "tui.select.up")) {
			if (this.flattenedFilteredTodos.length === 0) return;
			this.selectedIndex =
				this.selectedIndex === 0 ? this.flattenedFilteredTodos.length - 1 : this.selectedIndex - 1;
			this.updateList();
			return;
		}
		if (kb.matches(keyData, "tui.select.down")) {
			if (this.flattenedFilteredTodos.length === 0) return;
			this.selectedIndex =
				this.selectedIndex === this.flattenedFilteredTodos.length - 1 ? 0 : this.selectedIndex + 1;
			this.updateList();
			return;
		}
...
function renderTodoHeading(theme: Theme, todo: TodoFrontMatter, currentSessionId?: string, depth = 0, subtaskStats?: { total: number; closed: number }): string {
	...
	const subtaskText = subtaskStats ? theme.fg("muted", ` ${subtaskStats.closed}/${subtaskStats.total} subtasks`) : "";
	return (
		indent +
...
function renderTodoList(
	...
	const pushSection = (label: string, sectionTodos: TodoFrontMatter[]) => {
		...
		const childrenByParent = buildTodoHierarchy(sectionTodos);
		const flattened = flattenTodoHierarchy(sectionTodos, childrenByParent);
		const maxItems = expanded ? flattened.length : Math.min(flattened.length, 3);

		for (let i = 0; i < maxItems; i++) {
			const { todo, depth, subtaskStats } = flattened[i];
			const line = renderTodoHeading(theme, todo, currentSessionId, depth, subtaskStats);
			lines.push(`  ${line}`);
		}

		if (!expanded && flattened.length > maxItems) {
			lines.push(theme.fg("dim", `  ... ${flattened.length - maxItems} more`));
		}
	};
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description:
			...
			"Use `parent_id` on create/update to create or re-parent subtasks. " +
			"Title is the short summary; body is long-form markdown notes (update replaces, append adds). " +
			"Todo ids are shown as TODO-<hex>; id parameters accept TODO-<hex> or raw hex. " +
			"Claim tasks before working on them; close when complete.",
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			switch (action) {
				...
				case "create": {
					...
					if (params.parent_id !== undefined && params.parent_id.trim()) {
						const parentValidation = validateTodoId(params.parent_id);
						if ("error" in parentValidation) {
							return {
								content: [{ type: "text", text: "Invalid parent_id. Expected TODO-<hex>." }],
								details: { action: "create", error: "invalid parent_id" },
							};
						}
				...
				case "update": {
					...
					if (params.parent_id !== undefined) {
						if (!params.parent_id.trim()) {
							...
							if ("error" in parentValidation) {
								return {
									content: [{ type: "text", text: "Invalid parent_id. Expected TODO-<hex>." }],
									details: { action: "update", error: "invalid parent_id" },
								};
							}
							normalizedParentId = parentValidation.id;
						}
					...
					const result = await withTodoLock(todosDir, normalizedId, ctx, async () => {
						...
						if (params.parent_id !== undefined) {
							existing.parent_id = normalizedParentId;
						}
