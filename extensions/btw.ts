...
class BtwOverlay extends Container implements Focusable {
	...
	private transcriptTotalLines = 0;
	...
	handleInput(data: string): void {
		...
		if (this.keybindings.matches(data, "tui.select.pageUp")) {
			this.scrollTranscriptBy(this.transcriptHeight || 1);
			return;
		}
	...
	private getMaxScroll(): number {
		return Math.max(0, this.transcriptTotalLines - this.transcriptHeight);
	}

	private scrollTranscriptBy(deltaFromBottom: number): void {
		const maxScroll = this.getMaxScroll();
		this.transcriptScrollOffset = Math.max(0, Math.min(this.transcriptScrollOffset + deltaFromBottom, maxScroll));
		this.tui.requestRender();
	}

	private scrollToTop(): void {
		this.transcriptScrollOffset = this.getMaxScroll();
		this.tui.requestRender();
	}
	...
	override render(width: number): string[] {
		...
		this.transcriptTotalLines = transcript.length;
		const maxScroll = this.getMaxScroll();
		this.transcriptScrollOffset = Math.max(0, Math.min(this.transcriptScrollOffset, maxScroll));
		const startIndex = Math.max(0, transcript.length - transcriptHeight - this.transcriptScrollOffset);
		const visibleTranscript = transcript.slice(startIndex, startIndex + transcriptHeight);
		...
		const scrollInfo =
			this.transcriptTotalLines > this.transcriptHeight
				? ` · ${Math.min(this.transcriptTotalLines, startIndex + 1)}-${Math.min(this.transcriptTotalLines, startIndex + this.transcriptHeight)}/${this.transcriptTotalLines}`
				: "";
		lines.push(this.frameLine(this.theme.fg("dim", `Enter submit · Esc close · PgUp/PgDn/Home/End scroll${scrollInfo}`), innerWidth));
